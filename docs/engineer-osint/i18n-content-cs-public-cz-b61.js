(function(){
  ...
  for(const x of leads){
    if((x?.lead_id||x?.id)!=='B61-LEAD-01')continue;
    if(x.topic_cs===undefined||x.topic_cs===null||x.topic_cs===''){
      x.topic_cs='Denní ukrajinský údaj o ztrátách UGV/NRK zachycený na sekundární veřejné stránce Telegramu';
      translated++;
      processed.add('B61-LEAD-01');
    }
    x.translation_status_cs=x.translation_status_cs||'ANALYST_TRANSLATION';
    x.translation_provenance_cs=x.translation_provenance_cs||'ENGINEER_OSINT_TRANSLATION_LAYER';
  }
  ...
  D.translation_audit_cs.batches.push({batch:'2026-08-23-b61-public-cz',processed_ids:processedIds,fully_translated:processedIds.length,partially_translated:0,review_needed:0,scope:'PUBLIC-CZ-UI: complete B61 Czech presentation for bridge-assessment doctrine record, its two public source titles and the retained B61 Telegram lead topic; factual/base data and English presentation remain unchanged.',english_preserved:true,base_preserved:true});
