(function(){
  ...
  const STATIC_FALLBACK={
    ...
    'Ukraine local remote-mining organization':'Ukrajinská místní organizace dálkového minování','initial-medium':'počáteční – střední','JGSDF 8th current equipment matrix':'JGSDF 8. divize – aktuální matice techniky','initial-partial (07MSB association strengthened)':'počáteční – částečné (vazba na 07MSB posílena)','JGSDF 4th/10th current equipment matrix':'JGSDF 4./10. divize – aktuální matice techniky','weak-partial':'slabé – částečné','Poland Patrol20 current primary C2':'Polsko Patrol20 – aktuální primární C2','weak/open':'slabé / otevřené','NATO EOC identifiers':'Identifikátory NATO EOC','partial/open':'částečné / otevřené','Russia current primary visual coverage':'Rusko – aktuální primární vizuální pokrytí',weak:'slabé'
  };
  const uiCs=()=>({...STATIC_FALLBACK,...(window.__ENGINEER_I18N__?.ui?.cs||{})});
  function csPattern(t){let m;if((m=t.match(/^(\d+) records · (\d+) key · (\d+) archived$/i)))return `${m[1]} záznamů · ${m[2]} klíčových · ${m[3]} archivovaných`;if((m=t.match(/^(\d+) records · (\d+) visuals · (\d+) leads · (\d+) warnings$/i)))return `${m[1]} záznamů · ${m[2]} vizuálů · ${m[3]} leadů · ${m[4]} varování`;if((m=t.match(/^(\d+) canonical runs · (\d+) sources · (\d+) doctrine logical records$/i)))return `${m[1]} kanonických běhů · ${m[2]} zdrojů · ${m[3]} logických záznamů doktríny`;if((m=t.match(/^(\d+) count discrepancy · (\d+) ID collision · (\d+) dangling relations$/i)))return `${m[1]} nesoulad počtu · ${m[2]} kolize ID · ${m[3]} neukotvených vazeb`;if((m=t.match(/^(\d+)\s*%\s*(?:current|aktuálních)\s+canonical references\s+(?:materialized|materializováno)$/i)))return `${m[1]} % aktuálních kanonických referencí materializováno`;return null}
  function repairStatic(lang){const map=uiCs(),rev=new Map(Object.entries(map).filter(([,v])=>typeof v==='string').map(([k,v])=>[v,k]));const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);while(walker.nextNode()){const n=walker.currentNode,el=n.parentElement;if(!el||el.closest('#engineerLanguageSwitch,script,style'))continue;const raw=n.nodeValue||'',t=raw.trim();if(!t)continue;if(!staticOriginals.has(n))staticOriginals.set(n,raw);let to;if(lang==='cs')to=map[t]??map[t.toUpperCase()]??csPattern(t);else to=rev.get(t);if(typeof to==='string'&&to!==t){const lead=raw.match(/^\s*/)?.[0]||'',trail=raw.match(/\s*$/)?.[0]||'';n.nodeValue=lead+to+trail}else if(lang!=='cs'&&staticOriginals.has(n)){n.nodeValue=staticOriginals.get(n);staticOriginals.delete(n)}}}
  function repair(lang){for(const box of document.querySelectorAll('[data-open],article,.item'))repairBox(box,lang);const d=document.getElementById('detailContent');if(d)repairBox(d,lang);repairStatic(lang)}
