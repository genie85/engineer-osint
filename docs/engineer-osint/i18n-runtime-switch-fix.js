(function(){
  ...
  const pairs=(e,lang)=>{const keys=['title','summary','update_summary','description','note','topic','signal','assessment','next_action','why_it_matters','staff_relevance','training_relevance','operational_evidence','training_evidence','testing_evidence','what_it_does_not_prove','analytical_interpretation','fact','analysis','limit'];const out=[];for(const k of keys){const en=value(e,k,'en'),cs=value(e,k,'cs'),to=value(e,k,lang);for(const from of [en,cs])if(typeof from==='string'&&typeof to==='string'&&from.trim()&&to.trim()&&from.trim()!==to.trim())out.push([from.trim(),to.trim()])}for(const c of e?.claims||[]){const en=c.text_en??c.__orig_text??c.text??'',cs=c.text_cs??c.text??c.text_en??'',to=lang==='cs'?cs:en;for(const from of [en,cs])if(typeof from==='string'&&typeof to==='string'&&from.trim()&&to.trim()&&from.trim()!==to.trim())out.push([from.trim(),to.trim()])}return out};
