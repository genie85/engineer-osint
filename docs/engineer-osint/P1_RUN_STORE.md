...
Run `engineer-osint-20260826-B88` is the sole approved exception to the preceding eligibility rule. Its immutable source mixed a lead update into the record `UPDATE` count and carried two source identifiers absent from both canonical source registries. The published representation normalizes only those strict-intake defects, exposes the original Drive file ID and SHA-256 under `extensions.intake_normalization_v1`, and is covered by a dedicated regression test and attestation. This migration does not create a reusable waiver mechanism; later schema, count or reference failures remain ineligible.
