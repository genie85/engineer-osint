...
for (const entry of batch.entries) {
  assert.equal(entry.status, 'SOURCE_FOUND', `${entry.card_id} must remain SOURCE_FOUND when compatible redistribution rights are not demonstrated`);
  ...
  assert.match(entry.license_evidence, /does not (?:demonstrate|state) a compatible open redistribution licence|no compatible open redistribution licence/i);
...
assert.match(byId.get('ENG-TECH-0031').identity_evidence, /NOVO.*EOD|NOVO digital radiography/i);
assert.match(byId.get('ENG-TECH-0031').origin_url, /zenijnipluk\.mo\.gov\.cz/);
assert.match(byId.get('ENG-TECH-0032').identity_evidence, /NEO-1.*remote detection|NEO-1.*explosive/i);
assert.match(byId.get('ENG-TECH-0032').origin_url, /mod\.gov\.ua/);
assert.match(byId.get('ENG-TECH-0036').identity_evidence, /SIRKO-S1.*mine-laying.*demining|SIRKO-S1.*demining/i);
assert.match(byId.get('ENG-TECH-0037').identity_evidence, /Bizon-L.*engineering obstacles.*area mining|Bizon-L.*area mining/i);
assert.match(byId.get('ENG-TECH-0039').identity_evidence, /Système de dépollution de zone.*SDZ|SDZ.*teleoperated/i);
assert.match(byId.get('ENG-TECH-0039').author_rightsholder, /Nicolas Haeussler.*Armée de Terre.*Défense/i);
...
assert.equal(queue.items[0].card_id, 'ENG-TECH-0040', 'the next remote-visual unassessed card must remain ahead of zero-visual cards');
