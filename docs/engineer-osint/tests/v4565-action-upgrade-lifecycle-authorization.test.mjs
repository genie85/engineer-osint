...
const expectedTests=new Map([
  ['docs/engineer-osint/tests/v4556-workflow-lifecycle-helper.mjs','3501017e228ac37a59c2b1ec115786550fb014fb'],
  ['docs/engineer-osint/tests/v4557-browser-digest-normalization-hotfix.test.mjs','2999eecf4b6f45a98e674d7e4529da763a11837c'],
  ['docs/engineer-osint/tests/v4562-active-node24-migration.test.mjs','8c8612c73f677064db2d59f013d3bfecd6dfbcfe'],
  ['docs/engineer-osint/tests/v4563-action-node24-authorization.test.mjs','459c52e39ad732508bd8df8fa03793521803f04e']
]);
...
test('v4.5.65 authorizes exactly four immutable test baselines',()=>{
  ...
  for(const item of policy.authorized_test_files){
    assert.equal(expectedTests.get(item.file),item.historical_git_blob_sha,`${item.file}: authorization target mismatch`);
    assert.equal(gitBlobSha(readFileSync(item.file,'utf8')),item.historical_git_blob_sha,`${item.file}: baseline drift in authorization slice`);
...
test('v4.5.65 pins only exact v4.5.62 to diagnostic v4.5.64 workflow successors',()=>{
  ...
  for(const [file,v4562Sha] of expectedSuccessors){
    const current=gitBlobSha(readFileSync(`.github/workflows/${file}`,'utf8'));
    assert.equal(current,v4562Sha,`${file}: workflow mutation occurred in authorization slice`);
  }
