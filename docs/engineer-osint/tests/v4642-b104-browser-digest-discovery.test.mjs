...
const reconstructB103=(temp)=>{
  ...
  if(manifest.runs.at(-1)?.run_id===b105RunId){
    ...
    assert.equal(entry.parent_canonical_sha256,correctedCanonicalSha);
...
// Browser discovery predates its later authorization. Keep that proof independent: validate
// the exact candidate in dry-run mode, then materialize only in the disposable historical fixture.
const materializeHistoricalDryRun=(temp)=>{
  const tempRoot=join(temp,root);
  const plan=JSON.parse(runNode(temp,`${root}/append-run.mjs`,candidatePath));
...
test('v4.6.42 discovers exact normalized browser DOM digest for simulated historical B104 without authoritative writes',()=>{
  ...
  if(live.report.current_run_id===parentRunId){
    ...
    const b104Entry=live.manifest.runs.find(item=>item.run_id===runId);
    assert.ok(b104Entry,'exact corrected B104 ancestor missing under B105');
    assert.equal(b104Entry.canonical_sha256,correctedCanonicalSha);
  } else {
    assert.equal(live.report.current_run_id,runId,'canonical head is outside exact B103→corrected B104→B105 lifecycle');
  ...
  try{
    ...
    const append=materializeHistoricalDryRun(temp);
