...
test('v4.6.74a pins exact source, replacement, historical-next, corrected-B105 and authorized postwrite-repair successor for both guards',()=>{
  ...
  for(const target of auth.authorized_targets){
    ...
    assert.ok([target.source_git_blob_sha,target.replacement_successor_git_blob_sha,historicalNext,correctedB105,repairTarget.successor_git_blob_sha].includes(blob),`${target.path} must be an exact pinned lifecycle state`);
  }
});
