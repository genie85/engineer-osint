...
test('v4.5.94 preserves the exact B100 guard while permitting only exact B101/B102/executor successors',()=>{
  ...
  if(current===exactB101AppendSuccessor) assert.match(appendRaw,/guardedB101='engineer-osint-20260902-B101'/);
  if(current===exactB102AppendSuccessor||current===exactExecutorAppendSuccessor) assert.match(appendRaw,/guardedB102='engineer-osint-20260902-B102'/);
  if(current===exactExecutorAppendSuccessor) assert.match(appendRaw,/Explicit append authorization required for unrecognized write run/);
});
