...
const expectedTests=new Map([
  [`${root}/tests/v4557-browser-digest-normalization-hotfix.test.mjs`,'9939fbcbf9c7f09f61a9a5c82c795873bddd0a61'],
  [`${root}/tests/v4562-active-node24-migration.test.mjs`,'c61540fa8c9be9cb21129e46fe488391502102dd'],
  [`${root}/tests/v4563-action-node24-authorization.test.mjs`,'558b54212ff856a01be7c1a5dedfaa871e5c820c'],
  [`${root}/tests/v4565-action-upgrade-lifecycle-authorization.test.mjs`,'a74bdbbf767cdc986862da828ee394cfc09b3334'],
  [`${root}/tests/v4619-b103-public-cz-authorization.test.mjs`,'9bd857b2f9bb785fd4fcb75be3697bb18712fdd1'],
  [`${root}/tests/v4643-b104-wave2-local-image-authorization.test.mjs`,'0d73c824951f3eade09b24f5b59a389fb67b6d33'],
  [`${root}/tests/v4646-b104-cc0-authorization.test.mjs`,'87656fdd7ceccdf55bdb2ceac23093a694165a3b'],
  [`${root}/tests/v4647-b104-browser-digest-successor.test.mjs`,'233793d6acb49931b52f56d1543a7b92e9e3b3f6']
]);
...
test('v4.6.60 pins exactly the eight source test blobs discovered by the failed exact-head CI',()=>{
  ...
  for(const item of auth.authorized_test_successors){
    assert.equal(item.source_git_blob_sha,expectedTests.get(item.path),item.path);
    assert.equal(gitBlobSha(readFileSync(item.path,'utf8')),item.source_git_blob_sha,`${item.path}: source test drift before compatibility implementation`);
  }
});

test('v4.6.60 authorization remains predecessor-only and does not smuggle workflow/helper implementation',()=>{
  assert.equal(gitBlobSha(readFileSync(workflowPath,'utf8')),workflowPre);
  assert.equal(gitBlobSha(readFileSync(helperPath,'utf8')),helperPre);
