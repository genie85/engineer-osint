...
test('v4.5.57 historical browser successor remains pinned while B100/B101/B102/B103 have exact descendants',()=>{
  ...
  assert.ok([b100Successor,b101Successor,b102Successor,b103Successor].includes(gitBlobSha(workflow)));
  assertV4557Applied();
});

test('v4.5.57 keeps historical digest and exact B100/B101/B102/B103 fail-closed descendant digests',()=>{
  ...
  if([b101Successor,b102Successor,b103Successor].includes(gitBlobSha(workflow)))assert.match(workflow,/'engineer-osint-20260902-B101':'c8c134daff25a15b3825680f5e033d83a833f87910e2c94421adf634ee7a7acd'/);
  if([b102Successor,b103Successor].includes(gitBlobSha(workflow)))assert.match(workflow,/'engineer-osint-20260902-B102':'5122d347541c53638a59c8f3c855c417db8ae2ea5a04b002948d655d91b5e6d7'/);
  if(gitBlobSha(workflow)===b103Successor)assert.match(workflow,/'engineer-osint-20260902-B103':'68892883c8acc3dbdd7d9acc2e2d48682ac61008ad8b8a49f55c01fbef71e87a'/);
