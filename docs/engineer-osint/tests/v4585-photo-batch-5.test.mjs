...
const batch = JSON.parse(fs.readFileSync(path.join(batchDir, 'v4585.json'), 'utf8'));
const expectedIds = ['ENG-TECH-0024', 'ENG-TECH-0025', 'ENG-TECH-0026', 'ENG-TECH-0027', 'ENG-TECH-0030'];

assert.deepEqual(batch.entries.map((entry) => entry.card_id), expectedIds, 'v4.5.85 batch must contain the deterministic next five remote-visual unassessed cards');
...
assert.match(byId.get('ENG-TECH-0024').identity_evidence, /TALON 5.*Dragon Runner 20|Dragon Runner 20/i);
assert.match(byId.get('ENG-TECH-0024').origin_url, /acr\.mo\.gov\.cz/);
assert.match(byId.get('ENG-TECH-0025').identity_evidence, /Katyusha.*combat engineers|mine clearance/i);
assert.match(byId.get('ENG-TECH-0025').origin_url, /tass\.com/);
assert.match(byId.get('ENG-TECH-0026').identity_evidence, /MS-21/);
assert.match(byId.get('ENG-TECH-0026').origin_url, /vepozs\.mo\.gov\.cz/);
assert.match(byId.get('ENG-TECH-0027').identity_evidence, /PMS pontoon bridge set|PMS/i);
assert.match(byId.get('ENG-TECH-0027').origin_url, /acr\.mo\.gov\.cz/);
assert.match(byId.get('ENG-TECH-0030').identity_evidence, /Kuryer.*route-clearance|Kuryer.*engineer/i);
assert.match(byId.get('ENG-TECH-0030').origin_url, /t\.me\/s\/mod_russia/);
...
const queueReport = {
  ...
  items: [
    ...
    { card_id: 'ENG-TECH-0031', title: 'next remote-visual unassessed fixture', local_images: [], remote_visual_count: 1, review_status: null },
...
assert.equal(queue.items[0].card_id, 'ENG-TECH-0031', 'remote-visual unassessed cards must remain ahead of zero-visual cards');

console.log('v4.5.85 five-card SOURCE_FOUND photo batch test passed');
