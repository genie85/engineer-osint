...
4. Run `validate-patch.mjs`, all tests, build, runtime audit, PUBLIC-CZ audit and canaries before opening the draft PR.
...
Report `RUN_STORE_PARENT`, `RUN_STORE_PARENT_CANONICAL_SHA256`, `APPEND_ONLY_RUN_FILE`, `RUN_FILE_SHA256`, `RESULT_CANONICAL_SHA256`, `MANIFEST_APPEND_STATUS`, `B11_FROZEN_UNCHANGED` and the separate validation/build/deploy/read-back statuses.
