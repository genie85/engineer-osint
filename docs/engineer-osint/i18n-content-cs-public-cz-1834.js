...
(function(){
  ...
  const done=[];
  if(setMissing('ENG-VIS-0001',{title_cs:'Aktivační ceremoniál HHC, 1. ženijní brigády, 1. obrněné divize',analysis_cs:'Klíčové obrazové potvrzení změny struktury sil, nikoli demonstrace výkonnosti schopnosti.',what_is_visible_cs:'NEPROVEDENA PŘÍMÁ KONTROLA PIXELŮ. Oficiální popisek U.S. Army identifikuje vojáky v sestavě během aktivačního ceremoniálu HHC.'}))done.push('ENG-VIS-0001');
  if(setMissing('ENG-VIS-0002',{title_cs:'Výcvik potápěčů British Army 22 Engineer Regiment, Royal Navy EOD&S a EOD ukrajinského námořnictva při Sea Breeze 26-2',analysis_cs:'Vysoce hodnotný obrazový důkaz institucionalizovaného mnohonárodního odborného výcviku propojujícího vojenské ženijní schopnosti a EOD.',what_is_visible_cs:'NEPROVEDENA PŘÍMÁ KONTROLA PIXELŮ. Oficiální popisek Sixth Fleet uvádí, že potápěči British Army 22 Engineer Regiment a Royal Navy 33 EOD&S cvičí s personálem EOD ukrajinského námořnictva na člunu RHIB.'}))done.push('ENG-VIS-0002');
  if(setMissing('ENG-VIS-0003',{title_cs:'Integrovaná odminovací vozidla — vševojsková brigáda 77. skupiny armád PLA, konec července 2026',analysis_cs:'Vysoce hodnotná obrazová metadata pro následné určení varianty; přímá kontrola snímku zůstává otevřeným úkolem.',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B04 přímo kontrolovány.'}))done.push('ENG-VIS-0003');
  if(setMissing('ENG-VIS-0004',{title_cs:'REBS spouštěný z přívěsu — ženijní odřad 71. skupiny armád PLA, duben 2024',analysis_cs:'Klíčový obrazový podklad pro ověření, zda označení „REBS“ zahrnuje více čínských mostních variant nebo konfigurací.',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B04 přímo kontrolovány.'}))done.push('ENG-VIS-0004');
  if(setMissing('ENG-VIS-0005',{title_cs:'Pontonový prvek 73. skupiny armád buduje 300m plovoucí most — cvičení 2023',analysis_cs:'Užitečný referenční podklad k mostní schopnosti, uváděné výkony však zůstávají tvrzením oficiálního zdroje.',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — obrazové snímky videa nebyly v běhu B04 přímo kontrolovány.'}))done.push('ENG-VIS-0005');
  if(setMissing('ENG-VIS-0006',{title_cs:'Výcvik PLAA v překonávání překážek s osádkovými a bezosádkovými prostředky — duben 2026',analysis_cs:'Užitečný důkaz technologického signálu, nikoli přímý záznam vizuální identifikace konkrétního systému.',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — obrazové snímky videa nebyly v běhu B04 přímo kontrolovány.'}))done.push('ENG-VIS-0006');
  if(setMissing('ENG-VIS-0007',{title_cs:'Vozidlový minovací systém — brigáda 81. skupiny armád PLA, prosinec 2023',analysis_cs:'Klíčový identifikační podklad pro novou technickou kartu schopnosti ztížení pohybu při zachování nejistoty ohledně přesného modelu.',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B05 přímo kontrolovány.'}))done.push('ENG-VIS-0007');
  if(setMissing('ENG-VIS-0008',{title_cs:'Výcvik ženijních prostředků 81. skupiny armád — terminologie pásového integrovaného odminovače a raketového minovacího vozidla',analysis_cs:'Vysoce hodnotný obrazový podklad pro terminologii a organizaci; před identifikací na úroveň varianty je stále nutná přímá kontrola pixelů.',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímků na stránce nebyly v běhu B05 přímo kontrolovány.'}))done.push('ENG-VIS-0008');
  if(setMissing('ENG-VIS-0009',{title_cs:'Zkouška prostředku U.S. Army pro vytváření průchodu dopravovaného dronem',analysis_cs:'Vysoce hodnotné obrazové potvrzení pouze pro demonstrátor koncepce.',what_is_visible_cs:'Přímá kontrola obnovená v B06 zaznamenala vojáky, těžký vícerotorový UAS a ženijní vozidlo v pozadí.'}))done.push('ENG-VIS-0009');
  if(setMissing('ENG-VIS-0010',{what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0010');
  if(setMissing('ENG-VIS-0011',{what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0011');
  if(setMissing('ENG-VIS-0012',{what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0012');
  if(setMissing('ENG-VIS-0013',{what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0013');
  if(setMissing('ENG-VIS-0014',{title_cs:'Oficiální obrazové materiály Türkiye k 240m mostu přes Eufrat',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0014');
  if(setMissing('ENG-VIS-0015',{title_cs:'Oficiální obrazové materiály ROK Engineer School z výcviku s ostrými demolicemi',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0015');
  if(setMissing('ENG-VIS-0016',{title_cs:'Obrazové materiály ROK 1115 Engineer Group z výcviku UN TPP',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0016');
  if(setMissing('ENG-VIS-0017',{title_cs:'Oficiální produktové snímky DRDO BLT Arjun',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0017');
  if(setMissing('ENG-VIS-0018',{title_cs:'Oficiální produktové snímky DRDO CBRN Mini UGV',what_is_visible_cs:'NENÍ UVÁDĚNO JAKO PŘÍMÉ POZOROVÁNÍ — pixely snímku nebyly v běhu B10 přímo kontrolovány.'}))done.push('ENG-VIS-0018');
  window.__ENGINEER_PUBLIC_CZ_BATCH_1935__={processed_ids:done,mapped_fields:41,english_preserved:true};
})();
