...
## v4.5.3 curated provenance gate

`V453_PROVENANCE_REVIEW.json` records the manually curated primary-source review for the first three overlays that already have zero unexpected canonical residuals in v4.5.2. `audit-overlay-provenance.mjs` joins that registry to the generated v4.5.1 migration candidates and fails closed if a scoped candidate is unclassified, a candidate's source hints escape the reviewed source set, or a new `RICH-SRC-*` definition differs from its reviewed primary-source URL.

The provenance gate publishes:

- `overlay-provenance-audit.json`
- `overlay-provenance-audit.md`
- health markers `overlay_provenance_*`.

The gate deliberately separates evidence status instead of treating `source_id` presence as proof:

- `REVIEWED_SOURCE_BACKED` — the field wording was reviewed against the curated primary-source set;
- `REVIEWED_SOURCE_BACKED_WITH_ATTRIBUTION_LIMIT` — supported only with a required attribution/scope limitation, such as the EOD COE report not being represented as binding NATO policy;
- `SOURCE_DEFINITION_REVIEWED` / `SOURCE_LOCATOR_REVIEWED` — source metadata or locator matches the curated primary source;
- `ANALYTICAL_ROUTE_REQUIRED` — intelligence gaps, implications and similar analytical statements require the analytical/canonical-intelligence route rather than factual source canonicalization;
- `ADMINISTRATIVE_METADATA_REVIEW_REQUIRED` — run/provenance bookkeeping such as `last_verified_date` must be generated by the real production run rather than copied from an old overlay;
- `PRECISION_REVIEW_REQUIRED` — the underlying fact is supported but the persisted numerical/rounding precision needs correction or an explicitly scoped representation;
- `ABSENCE_REVIEW_REQUIRED` — the statement is based on an absence in a specific source and must not be generalized beyond that source;
- `EXPANDED_SOURCE_REVIEW_REQUIRED` — the current primary sources do not support the full overlay wording and an additional authoritative source or narrower value is required.

A `PASS` from v4.5.3 means the provenance review is **complete and internally consistent**, not that every candidate is migration-ready. The artifact always keeps `safe_to_append=false` and `safe_to_retire_overlays=false`. Precision, absence, expanded-source, analytical and administrative candidates remain explicit blockers for a production migration plan until their final values/routes are reviewed.

## Safety rule

The retirement audit, migration map, strict dry-run and provenance gate are read-only with respect to canonical data and persistence. They must never create a real run, alter `data/run-store-manifest.json`, fabricate a production hash, change a source/evidence/claim in persistent state, or turn runtime-only factual content into canonical truth. Actual migration must use the normal strict append-only run path and must be followed by a fresh retirement audit before any overlay is removed.
