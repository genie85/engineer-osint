...
for(const x of [...(patch.new_records||[]),...(patch.updated_records||[])]){if(!x?.id)continue;const old=rm.get(x.id)||{};const type=x.type||(x.id.startsWith('ENG-TECH-')?'ENG-TECH':x.id.startsWith('ENG-EVT-')?'ENG-EVT':x.id.startsWith('ENG-UNIT-')?'ENG-UNIT':x.id.startsWith('ENG-SIG-')?'ENG-SIG':old.type||'ENG-RECORD');const merged={...old,...x,type,run_id:old.run_id||patch.state.run_id,last_update_run:patch.state.run_id};if(x.summary&&!merged.analysis)merged.analysis=x.summary;if(x.summary&&!merged.fact)merged.fact=x.summary;if(x.source_ids)merged.source_ids=[...new Set([...(old.source_ids||[]),...x.source_ids])];const ots=old.temporal_observations||old.timeline_events||[],nts=x.temporal_observations||x.timeline_events||[];if(ots.length||nts.length){const tm=new Map();for(const t of [...ots,...nts]){const k=[t.event_date||t.event_date_from||t.date||'',t.observation_type||t.type||'',JSON.stringify(t.source_ids||[])].join('|');tm.set(k,{...(tm.get(k)||{}),...t})}merged.temporal_observations=[...tm.values()]}rm.set(x.id,merged)}
