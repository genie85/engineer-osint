(function(){
  ...
  const evidenceFor=(key,ids)=>sortRecent(evidence().filter(e=>geoObject(e,key,ids)));
  ...
  function renderHub(key){
    ...
    const tech=technologyRecords(xs).slice(0,8),developments=developmentRecords(xs).slice(0,8),ev=evidenceFor(key,ids).slice(0,8);
