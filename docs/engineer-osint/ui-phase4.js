...
function detail(){
  ...
  if(claims.length)h+=claims.map(c=>'<div class="item"><b>'+esc(c.classification||'CLAIM')+'</b> '+esc((lang()==='cs'?(c.text_cs||c.text||c.text_en):(c.text_en||c.text||c.text_cs))||'')+'<div class="muted">'+(c.source_ids||[]).map(sid=>{const s=S.get(sid);return s?.url?'<a href="'+esc(s.url)+'" target="_blank">'+esc(sid)+'</a>':esc(sid)}).join(', ')+'</div></div>').join('');
  ...
  if(r.type==='ENG-TECH'||r.type==='ENG-SIG')h+='<h4>'+cs('HISTORIE VYSPĚLOSTI','MATURITY HISTORY')+'</h4><div class="muted">'+esc(r.stage||r.maturity||'UNKNOWN')+' — '+cs('pouze explicitní aktuální bod; mezistupně se nedomýšlejí.','explicit current point only; intermediate stages are not inferred.')+'</div>';
