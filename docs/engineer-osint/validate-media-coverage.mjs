...
const sourceCandidates=runtimeSources.map(s=>{
  ...
  const relatedIds=asArray(s.related_ids);
  const metadata={title:Boolean(s.title||s.name),publisher:Boolean(s.publisher||s.organization),date:Boolean(s.publication_date),related:relatedIds.length>0};
  ...
  else if(c.asset_level&&!metadata.related) disposition='ASSET_URL_UNLINKED_TO_ENTITY';
  else if(c.asset_level) disposition='ASSET_URL_METADATA_INCOMPLETE';
  return {source_id:s.id||null,kind:c.kind,url,asset_level:c.asset_level,already_canonical:canonicalUrls.has(url),related_ids:relatedIds,metadata,disposition};
});
...
const sourceSummary={total:sourceCandidates.length,asset_level:sourceCandidates.filter(x=>x.asset_level).length,container_level:sourceCandidates.filter(x=>!x.asset_level).length,already_canonical:sourceCandidates.filter(x=>x.disposition==='ALREADY_CANONICAL').length,review_ready:sourceCandidates.filter(x=>x.disposition==='CANONICALIZATION_REVIEW_READY').length,metadata_incomplete:sourceCandidates.filter(x=>x.disposition==='ASSET_URL_METADATA_INCOMPLETE').length,unlinked_assets:sourceCandidates.filter(x=>x.disposition==='ASSET_URL_UNLINKED_TO_ENTITY').length};
const report={
