(function(){
  ...
  if(setLeadAll('LEAD-011',{next_action_cs:'Získat přímo kontrolovatelné veřejně dostupné oficiální podklady tam, kde je to technicky možné, a rozlišovat mezi přímou kontrolou a záznamy založenými pouze na popisku.'}))translated.push('LEAD-011');
  if(setLeadAll('LEAD-012',{next_action_cs:'Zjistit, zda jsou popisy REBS montovaného na vozidle a vypouštěného z přívěsu variantami jedné rodiny, nebo pouze obecným anglickým překladem; dohledat čínské označení a primární technická metadata.'}))translated.push('LEAD-012');
  if(setLeadAll('LEAD-013',{next_action_cs:'Použít oficiální čínské popisky, akviziční nebo výrobní materiály či autoritativní technické publikace; neodvozovat typové označení GSL/Type pouze z vzhledu.'}))translated.push('LEAD-013');
  if(setLeadAll('LEAD-014',{next_action_cs:'Získat přímo kontrolovatelné snímky nebo videozáběry klíčových ženijních systémů PLA při zachování právních a přístupových omezení; kontrolní součty počítat pouze u kopií, které lze legálně archivovat.'}))translated.push('LEAD-014');
  if(setLeadAll('LEAD-015',{next_action_cs:'Dohledat autoritativní podklady CMC/MND po roce 2020 k aktualizaci, nahrazení nebo pokračující platnosti; stav nevyvozovat pouze z pokračujícího koncepčního používání.'}))translated.push('LEAD-015');
  if(setLeadAll('LEAD-016',{next_action_cs:'Zjistit, zda zkušební verze z roku 2022 přešla do vyhlášené aktuální osnovy, jaký je její formální název a vydání a zda lze veřejně identifikovat podřízené osnovy specifické pro ženijní vojsko.'}))translated.push('LEAD-016');
  if(setLeadAll('LEAD-017',{next_action_cs:'Vyřešit formální modelové, typové nebo GSL označení z primárních zdrojů PLA/MND či výrobce; 火箭布雷车 / vozidlový systém kladení min ponechat jako popisné označení, dokud nebude potvrzeno.'}))translated.push('LEAD-017');
  if(setLeadAll('LEAD-018',{next_action_cs:'Zjistit aktuální vydání a datum ATP a vztah jeho rozsahu ke STANAG 2036 Edition 6; nevyvozovat aktuální procedurální obsah pouze z názvu v registru.'}))translated.push('LEAD-018');
  if(setLeadAll('LEAD-023',{note_cs:'V tomto běhu nebyl nalezen silnější aktuální primární organizační zdroj z roku 2026 pro polský Patrol 20.'}))translated.push('LEAD-023');
  if(setLeadAll('LEAD-024',{note_cs:'Dohledané veřejné údaje byly historické, nikoli aktuální stav pro rok 2026.'}))translated.push('LEAD-024');
  if(setLeadAll('LEAD-026',{note_cs:'U 8. praporu došlo k částečnému doplnění díky vizuálnímu přiřazení 07MSB.'}))translated.push('LEAD-026');
  if(setLeadAll('LEAD-027',{note_cs:'Samostatná stránka 2nd Division uvádí osádku 2, délku 7 630 mm, šířku 2 770 mm, hmotnost 25,0 t, rychlost 50 km/h a IHI Aerospace. Extrakce z centrální stránky techniky obsahuje sousední či podezřelé údaje podobné mostní technice a odlišné rozměry i výrobce. Prozatím považovat samostatnou stránku za čistší zdroj; před opravou uložených specifikací rozpor vyřešit podle oficiálního technického listu.'}))translated.push('LEAD-027');
  if(setLeadAll('LEAD-029',{note_cs:'Dvě aktuální stránky v jazyce výrobce se liší o 400 kg. Tvrzení výrobce o provozu a bojovém nasazení vyžadují nezávislé potvrzení.',next_action_cs:'Dohledat technickou dokumentaci Tureckých pozemních sil / MSB nebo nezávislé údaje o jednotkách či cvičeních; bez důkazu nevolit jednu z uváděných hmotností.'}))translated.push('LEAD-029');
  if(setLeadAll('LEAD-034',{next_action_cs:'Dohledat dokumentaci MO RF, jednotek nebo výrobce k testům či nezávislé obrazové podklady potvrzující přijímající jednotky, počty, konfiguraci a výsledek zkoušek v létě 2026.'}))translated.push('LEAD-034');
  if(setLeadAll('LEAD-043',{topic_cs:'Navazující výsledek po zrušené české zakázce na průzkumného pyrotechnického robota N006/26/V00000714',next_action_cs:'Pokračovat v hledání v NEN, VVZ a registru smluv s variantami předmětu a identifikátory zakázky; nevyvozovat, že byl požadavek opuštěn.'}))translated.push('LEAD-043');
  if(setLeadAll('LEAD-049',{topic_cs:'32nd EOD COE Steering Committee — schválený plán rozvoje 2027–2031 / změna struktury / E&T 2027',next_action_cs:'Zopakovat přímé načtení oficiálního odkazu; pokud bude stabilní, kanonizovat schválené závěry jednání jako fakt o řízení, organizaci a E&T, nikoli jako vyhlášení doktríny.'}))translated.push('LEAD-049');
  if(setLeadAll('LEAD-CZE-001',{next_action_cs:'Dohledat oficiální organizační, výcvikové nebo certifikační materiály; z náborových příspěvků neodvozovat počet týmů.'}))translated.push('LEAD-CZE-001');
  if(setLeadAll('LEAD-CZE-002',{next_action_cs:'Získat a přímo prověřit oficiální obrazové materiály 15. ženijního pluku a jeho praporů; přednostně mostní, EOD a techniku pro zprůchodňování tras (route clearance).'}))translated.push('LEAD-CZE-002');
  ...
  D.translation_audit_cs.batches.push({batch:'2026-08-21-2034-leads-023-049-cze',processed_ids:[...new Set([...translated,...qualityFixed,...review,'ENG-EVID-0131','ENG-EVID-0132','ENG-DOC-0042','ENG-EVID-0133'])],fully_translated:qualityFixed.length+translated.length+4,partially_translated:0,review_needed:review.length,scope:'PUBLIC-CZ-UI: LEAD-023, 024, 026, 027, 029, 034, 043, 049, LEAD-CZE-001 a LEAD-CZE-002 – doplnění bezpečně přeložitelných note/topic/next_action polí podle materializovaného anglického runtime. Oficiální názvy, zkratky a systémová označení zachována. EN pole a factual registry data zachována.',english_preserved:true});
  window.__ENGINEER_I18N_CONTENT_CS_PUBLIC_CZ__={translated_entities:[...new Set([...translated,...qualityFixed,'ENG-EVID-0131','ENG-EVID-0132','ENG-DOC-0042','ENG-EVID-0133'])],review_needed_entities:review,resolved_mapping_entities:['PUBLIC_REGISTRIES','PUBLIC_EXTERNAL_LEADS','PUBLIC_REGISTRY_ENUMS','PUBLIC_STATUS_ENUM','PUBLIC_CURRENT_VALUE_STATUS','PUBLIC_OBSERVATION_BASIS','PUBLIC_URL_VALIDATION_STATUS','B16_B17_TRENDS','B18_PUBLIC_ENUMS','B19_EUGS_ENUMS','B19_EVIDENCE_ENUMS','B20_WIT_ENUMS','P1_LEADS_001_018','PUBLIC_CZ_LEADS_023_049_CZE','PUBLIC_CZ_HYBRID_TEXT','PUBLIC_UI_ENUM_HYBRIDS','AUDIT_VM_SAFE'],version:'2.3',last_batch:'2026-08-21-2034-leads-023-049-cze'};
})();
